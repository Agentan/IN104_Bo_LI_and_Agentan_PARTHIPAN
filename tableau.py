...
class Tableau:
    ...
    def initialisation(self, bdd_path, id):
        for element in os.listdir(bdd_path):
            if element.endswith('.txt'):
                ...
                article = open(element_path, encoding='utf-8', errors='ignore') #param¨¨tres optionnels utilis¨¦s pour eviter des erreurs d'encodage
    ...
    def partie_log(self, document, word):
        # la partie log(N/dfi) ne change pas pour w_ij et w_iq, on d¨¦finit cette fonction pour ¨¦viter de la calculer deux fois
        try:
    ...
    def similarity(self, document, transf_query):  ##calculer la similarit¨¦ entre un document et la requ¨ºte
        A = 0;  # w_ij * w_iq
        B = 0;  # w_ij * w_ij
    ...
    def search(self):
        query = input('enter your query :') # demande de la requ¨ºte
        ...
        for document in reunion:
            result[document] = self.similarity(document, transf_query)  #calcule la similarit¨¦ entre la requ¨ºte et chaque document o¨´ appara?t un mot de la requ¨ºte
        ...
        for couple in sorted_result:
            print(self.ID_docs[couple[0]]) #affiche les r¨¦sultats de la recherche


def load(save_path): #charger l'index pr¨¦c¨¦demment enregistr¨¦
    f = open(save_path, 'rb')
    D = Tableau()
